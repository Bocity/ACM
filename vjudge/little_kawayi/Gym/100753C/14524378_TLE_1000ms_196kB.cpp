...
long double getSqrt(long double tmp) {
    long double X = 1.0;
...
int main() {
    ...
    while(1) {
        m = l + (r - l) / 2.0;
        long double tmp = ck(m, n);
        if(fabs(tmp - a) <= eps || r - l < eps) {
            ans = m;
            break;
        }
        else if(tmp - a > eps) {
