...
pair<int, int> f(int x1, int x2,int x3,int x4,int z, int s,int y, int x,int k, int zds)//四个象限，左上右下坐标， 当前k值, 每象限点数 
{
	...
	if (xx == 1)
		return f(x1,x4,x3,x2,z,s+x+1>>1,z+y>>1,x,k,zds>>2);
	else if (xx == 2)
		return f(x1,x2,x3,x4,z,s,z+y>>1,s+x>>1,k,zds>>2);
	else if (xx == 3)
		return f(x1,x2,x3,x4,z+y+1>>1,s,y,s+x>>1,k,zds>>2);
	else
		return f(x3,x2,x1,x4,z+y+1>>1,s+x+1>>1,y,x,k,zds>>2);
}
 main()
{
	cin.tie(0);
