...
struct kdtree {
    ...
    ll get(int rt, Node p) {
        ll tmp = 0;
        for (int i = 0; i < k; i++)
            tmp += max((ll)(p.fea[i] - data[rt].mn[i]) * (p.fea[i] - data[rt].mn[i]),
                       (ll)(data[rt].mx[i] - p.fea[i]) * (data[rt].mx[i] - p.fea[i]));
        return tmp;
    }
    ...
    void query(int rt, int now) {
        ...
        if (dl < dr) {
            if (dl < Q.top().first || Q.size() < m) query(data[rt].l, (now + 1) % k);
            if (dr < Q.top().first || Q.size() < m) query(data[rt].r, (now + 1) % k);
        } else {
            if (dr < Q.top().first || Q.size() < m) query(data[rt].r, (now + 1) % k);
            if (dl < Q.top().first || Q.size() < m) query(data[rt].l, (now + 1) % k);
        }
