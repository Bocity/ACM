...
void KDTree::Query(Node p, int m, int rt, int dept) {
    ...
    for (int i = 0; i < 2; i++) cur.first += Sqrt2(p.feature[i] - cur.second.feature[i]);
    ...
    //以下是回溯过程，维护一个优先队列
    if (Q.size() < 1) //如果队列没有满，则继续放入
    {
        if (cur.second.feature[2] <= p.feature[2]) Q.push(cur);
        fg = 1;
    } else {
        if (cur.second.feature[2] <= p.feature[2] &&
            (cur.first < Q.top().first || (cur.first == Q.top().first && cur.second.cost < Q.top().second.cost))) //如果找到更小的距离，则用于替换队列Q中最大的距离的数据
        {
            Q.pop();
            Q.push(cur);
        }
        if (Sqrt2(data[rt].feature[dim] - p.feature[dim]) < Q.top().first) {
            fg = 1;
        }
    }
...
int main() {
    ...
    while (T--) {
        ...
        while (t--) {
            ...
            //     Q.pop();
            // }
            printf("%d %d %d\n", Q.top().second.feature[0], Q.top().second.feature[1], Q.top().second.feature[2]);
        }
    }
    return 0;
}
