...
void KDTree::Query(Node p, int m, int rt, int dept) {
    ...
    for (int i = 0; i < k; i++) cur.first += (ll) Sqrt2((ll) cur.second.feature[i] - (ll) p.feature[i]);
...
void Print(Node data) {
    for (int i = 0; i < 3; i++) printf("%lld%c", data.feature[i], i == 2 ? '\n' : ' ');
}
...
int main() {
    ...
    while (T--) {
        ...
        while (mm--) {
            Node p;
            for (int i = 0; i < 3; i++) scanf("%lld", &p.feature[i]);
