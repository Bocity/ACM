...
struct Node {
    ...
    int id;           //出现顺序
} _data[N], data2[N]; //_data[]数组代表输入的数据
...
class KDTree {
  ...
  private:
    ...
    bool flag[4 * N]; //用于标记某个节点是否存在，1表示存在，-1表示不存在
} kd;
...
void KDTree::Build(int l, int r, int rt, int dept) {
    ...
    flag[lson] = flag[rson] = 0; //当前节点的孩子暂时标记不存在
...
void KDTree::Query(Node p, int m, int rt, int dept) {
    ...
    {
        if (cur.second.feature[0] != p.feature[0] || cur.second.feature[1] != p.feature[1]) Q.push(cur);
        fg = 1;
    } else {
        if (cur.first < Q.top().first && ((cur.second.feature[0] != p.feature[0] || cur.second.feature[1] != p.feature[1]))) //如果找到更小的距离，则用于替换队列Q中最大的距离的数据 。／／点使用限制
        {
            Q.pop();
            Q.push(cur);
        }
...
int main() {
    ...
    k = 2;
    while (T--) {
        while (scanf("%d", &n) != EOF) { // n个点 。 k维度
            for (int i = 0; i < n; i++)
                for (int j = 0; j < k; j++) scanf("%lld", &_data[i].feature[j]);
            for (int i = 0; i < n; i++) data2[i] = _data[i];
            kd.init();
            kd.Build(0, n - 1, 1, 0);
            int t, m;
            m = 1;
            ...
            while (t < n) { // t个询问
                ...
                ll anss = 0;
                // for (int i = 0; i < k; i++)
                //   anss += (ll) Sqrt2((ll) Q.top().second.feature[i] - (ll) p.feature[i]); //估计距离 容易爆
                printf("%lld\n", Q.top().first);
                Q.pop();
                t++;
            }
