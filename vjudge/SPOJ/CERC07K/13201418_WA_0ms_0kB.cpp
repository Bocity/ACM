...
int main()
{
	...
	while (cin >> n >> m && (n || m))
	{
		...
		while (!q.empty())
		{
			...
			if (g[now.n][now.m] == 'X')
			{
				ans = bs[now.n][now.m][vis[now.n][now.m]];
				break;
			}
			for (int i = 0; i < 4; i++)
			{
				...
				if (should(nxt.n, nxt.m, vis[now.n][now.m]))
				{
					...
					if (g[nxt.n][nxt.m] == 'b')
						...
						x |= 8;
                    if (bs[now.n][now.m][vis[now.n][now.m]] + 1 < bs[nxt.n][nxt.m][x])
                    {
                        vis[nxt.n][nxt.m]  = x;
                        bs[nxt.n][nxt.m][x] = bs[now.n][now.m][vis[now.n][now.m]] + 1;
					    q.push(nxt);
                    }
				}
			}
