...
int b[maxn], t[maxn], n, m, size, v[maxn], tot = 0, vis[maxn * 30], ann[maxn * 30];
struct chairtree {
    int l, r, size;
} a[maxn * 30];
...
int Ask(int lx, int rx) {
    ...
    if (a[rx].size - a[lx].size == lx - rx + 1) return a[rx].size - a[lx].size;
...
int main() {
    read(n);
    for (int i = 1; i <= n; i++) read(v[i]), b[i] = v[i];
    Hash1();
    t[0] = Build(1, size);
