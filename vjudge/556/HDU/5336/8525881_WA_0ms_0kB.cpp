    ...
    while (cin >> r >> c >> n >> T) {
        ...
        for (int t = 0; t <= T; t++) {
            ...
            for (int j = 1; j <= n + 1; j++) {
                ...
                if (No[j].up && Y - No[j].far > 0 && mp[X][Y - No[j].far]&&t!=T) {
                    index = mp[X][Y - No[j].far];
                    No[index].s++;
                    No[j].up = 0;
                    No[index].bt = t;
                }
                if (No[j].down && Y + No[j].far <= r && mp[X][Y + No[j].far]&&t!=T) {
                    index = mp[X][Y + No[j].far];
                    No[index].s++;
                    No[j].down = 0;
                    No[index].bt = t;
                }
                if (No[j].left && X - No[j].far > 0 && mp[X - No[j].far][Y]&&t!=T) {
                    index = mp[X - No[j].far][Y];
                    No[index].s++;
                    No[j].left = 0;
                    No[index].bt = t;
                }
                if (No[j].right && X + No[j].far <= c && mp[X + No[j].far][Y]&&t!=T) {
                    index = mp[X + No[j].far][Y];
                    No[index].s++;
                    No[j].right = 0;
                    No[index].bt = t;
