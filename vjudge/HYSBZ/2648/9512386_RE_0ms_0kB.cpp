...
class KDTree {
  ...
  private:
    Node data[3 * N]; // data[]数组代表K-D树的所有节点数据
    int flag[3 * N];  //用于标记某个节点是否存在，1表示存在，-1表示不存在
} kd;
...
void KDTree::Query(Node p, int m, int rt, int dept) {
    ...
    for (int i = 0; i < k; i++) cur.first += abs( cur.second.feature[i] - p.feature[i]);
    ...
    if (Q.size() < m) //如果队列没有满，则继续放入
    {
        ...
        if ( 2*abs( p.feature[dim] -  data[rt].feature[dim])< Q.top().first) {
            fg = 1;
        }
    }
...
int main() {
        ...
        scanf("%d %d",&n,&t);
        for(int i=0;i<n;i++)
        for (int j = 0; j < 2; j++) scanf("%d", &_data[i].feature[j]);
        kd.Build(0, n - 1, 1, 0);
        Node p;
        m = 1;
        int c;
        for(int i=0;i<t;i++){
            scanf("%d %d %d",&c,&p.feature[0],&p.feature[1]);
            if (c==1){
                kd.add(p,1,0);
            }
            if (c==2){
