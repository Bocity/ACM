...
struct Node {
    int fea[5], mn[5], mx[5], l = 0, r = 0; //定义属性数组
...
void KDTree::Query(Node p, int m, int rt, int dept) {
    ...
    if (dl < dr) {
        if (dl < ans) Query(p, m, data[rt].l, dept ^ 1);
        if (dr < ans) Query(p, m, data[rt].r, dept ^ 1);
    } else {
        if (dr < ans) Query(p, m, data[rt].r, dept ^ 1);
        if (dl < ans) Query(p, m, data[rt].l, dept ^ 1);
    }
