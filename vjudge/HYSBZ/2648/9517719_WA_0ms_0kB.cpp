...
void KDTree::Build(int l, int r, int rt, int dept) {
    ...
    data[rt] = _data[mid];
    for (int i = 0; i < k; i++) data[rt].mn[i] = data[rt].mx[i] = data[rt].fea[i];
...
//     if (~flag[lson]) dl = cal(p, lson);
//     if (~flag[rson]) dr = cal(p, rson);
//     //  cout << dl << " " << dr << "dldr" << Q.top().first << "\n";
//     if (dl < dr) {
//         if (dl < Q.top().first) Query(p, m, lson, dim + 1);
//         if (dr < Q.top().first) Query(p, m, rson, dim + 1);
//     } else {
//         if (dr < Q.top().first) Query(p, m, rson, dim + 1);
//         if (dl < Q.top().first) Query(p, m, lson, dim + 1);
//     }
...
int main() {
    ...
    while (mm--) {
        ...
        if (opt == 1)
            ...
            kd.Query(pp, 1, 1, 0);
            printf("%lld\n", Q.top().first);
            // cout << "\n" << Q.top().second.fea[0] << " " << Q.top().second.fea[1] << " " << Q.top().first << endl;
        }
    }
    return 0;
}